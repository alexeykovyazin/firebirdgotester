...
-- lookup table: states of documents
...
create or alter view v_geo_cities as select 1 x from rdb$database;
create or alter view v_geo_countries as select 1 x from rdb$database;
create or alter view v_geo_continents as select 1 x from rdb$database;

recreate table geo_points (
  id dm_ids generated by default as identity constraint pk_geo_points primary key
  ,pid dm_ids
  ,name dm_name
  ,land_risk int -- for countries only: multiplier for delivery risk
  ,is_hub dm_sign -- for cities only: is this point serves as HUB for logistic chains ?
  ,hub_dist int -- for NON-hub cities: distance from hub city
  ,constraint fk_geo_points foreign key (pid) references geo_points(id) on delete cascade
);
create index geo_points_name on geo_points(name);

create or alter view v_geo_cities as
-- cities (end points, i.e. they have no 'childs'):
select -- g.*
     g.id
     ,g.name as city_name
     ,g.is_hub
     ...
     ,c.land_risk
     ,m.id as continent_id
     ,m.name as continent_name
from geo_points g
join geo_points c on g.pid = c.id
join geo_points m on c.pid = m.id
where not exists(
    select p.*
    from geo_points p
    where p.pid = g.id
...
create or alter view v_geo_countries as
-- countries: intermediate points which have at least one child which is CITY:
select c.* from geo_points c
where exists (
...
create or alter view v_geo_continents as
-- continents: their parent is 'Earth':
select t.* from geo_points t
where exists (
    select e.* from geo_points e
    where e.pid is null
    and e.id = t.pid
);


-- transp. cost between continents:
recreate table tariff_global (
  id dm_ids generated by default as identity constraint pk_tariff_global primary key
  ,src_continent_id dm_ids
  ,tgt_continent_id dm_ids
  ,price_per_unit dm_cost
);
create index tariff_global_src_tgt on tariff_global(src_continent_id, tgt_continent_id);

-- transp. cost between hub-cities of countries belonging same continent:
recreate table tariff_ground (
  id dm_ids generated by default as identity constraint pk_tariff_ground primary key
  ,src_country_id dm_ids
  ,tgt_country_id dm_ids
  ,price_per_unit dm_cost
);
create index tariff_ground_src_tgt on tariff_ground(src_country_id, tgt_country_id);

-- transp. cost within country (from hub-city to non-hub, or within hub-city):
recreate table tariff_local (
  id dm_ids generated by default as identity constraint pk_tariff_local primary key
  ,src_city_id dm_ids
  ,tgt_city_id dm_ids
  ,price_per_unit dm_cost
);
...
-- 11.04.2022
alter table agents
   add constraint fk_agents_geo_points foreign key (city_id) references geo_points(id)
;
...
begin
    ...
    begin
        ...
        begin
            ...
            begin
                -- temp, 19.11.2025
                --rdb$set_context('USER_SESSION','DBG_QD_ROWS_FOR_DOC_BEFORE_FETCH',
                --    'WHERE EXPR: '
                --    ||' q.ware_id = '|| coalesce( :v_dd_ware_id, '<?>')
                --    || ' and q.snd_optype_id = ' || coalesce( :a_old_optype, '<?>')
                --    || ' and q.rcv_optype_id = ' || coalesce( :v_old_rcv_optype, '<?>')
                --    || ' and q.worker_id is not distinct from ' || coalesce( fn_this_worker_seq_no(), 'NULL')
                --    || ' and q.snd_id = ' || coalesce( :v_dd_id, '<?>')
                --);

                fetch c_qd_rows_for_doc into v_id;
                if ( row_count = 0 ) then leave;

                -- temp, 19.11.2025 rdb$set_context('USER_SESSION','DBG_QD_ROWS_FOR_DOC', 'v_id=' || v_id);
...
begin
    ...
    do begin
        ...
        begin
            ...
            --  temp, 19.11.2025 rdb$set_context('USER_SESSION','DBG_ADD_TMP_DEP_DOCS', 'a_base_doc_id=' || a_base_doc_id || ', dependend_doc_id=' || :v_dependend_doc_id);
        when any do
            -- ::: nb ::: do NOT use "wh`en gdscode <mnemona>" followed by "wh`en any":
            -- the latter ("w`hen ANY") will handle ALWAYS, even if "w`hen <mnemona>"
            -- catched it's kind of exception!
...
as
    ...
    declare v_affects_on_inventory_balance dm_sign NOT NULL;
...
begin
    ...
    -- 18.11.2025 NB: 'v_affects_on_inventory_balance' must be NOT null.
    -- Otherwise hard-to-detect error can occur with attempt to store
    -- negative remainders in invnt_saldo. Found for query:
    -- "select ... from optypes o where o.id in( :v_old_op, :v_new_op )"
    -- when v_new_op was null.
    -- Due to regression in 6.x, this query did not returned record and
    -- 'v_affects_on_inventory_balance' remained null.
    -- This, in turn, prevented execution of 'sp_kill_qty_storno'.
    -- See https://github.com/FirebirdSQL/firebird/issues/8805
    ...
    -- temp, 19.11.2025
    --rdb$set_context('USER_SESSION','TRG_DOC_LIST_BIUD',
    --    'v_old_op = ' || coalesce(v_old_op, '<NULL>')
    --    || ', v_new_op = ' || coalesce(v_new_op, '<NULL>')
    --    || ', RESULT: v_affects_on_inventory_balance=' || coalesce(v_affects_on_inventory_balance, '<NULL>')
    --);
...
create or alter procedure srv_predictable_unit_choice(
...
begin
    ...
    select cast( o.unit as char(80) ) || cast( o.predictable_selection_priority as char(11) )
    from business_ops o
    where
        o.predictable_selection_priority > cast( substring( :v_prev from 81 ) as int )
    ...
    order by o.predictable_selection_priority
    rows 1
    into v_prev;
    if (row_count = 0) then
    begin
        ...
        from business_ops o
        where
            --o.unit <> :c_unit_for_recalc_idx_stat
            ( fn_internal_enable_mon_query() = 1 or o.unit <> :c_unit_for_gather_mon_data ) -- do NOT choose srv_fill_mon if mon query disabled
        and ( :a_included_modes = '' or :a_included_modes||',' containing trim(o.mode)||',' )
        and ( :a_included_kinds = '' or :a_included_kinds||',' containing trim(o.kind)||',' )
        and ( :a_excluded_modes = '' or :a_excluded_modes||',' NOT containing trim(o.mode)||',' )
        and ( :a_excluded_kinds = '' or :a_excluded_kinds||',' NOT containing trim(o.kind)||',' )
